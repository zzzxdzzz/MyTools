; Compute the effective emission angle
; It takes parameters (i, e, psi, theta), returns the
; effective emission angle.
...
;   found a bug: when theta=0, it returns i rather than e.
;   changed the infinity value from 1e38 to !values.d_infinity
;

function effemi, i, e, psi0, theta, cos=cos
; if set keyword 'cos', then all i, e, and returned effective
; emission angle are the cosines of the corresponding angles

  if theta eq 0 then return, e
  ...
  if w1[0] ne -1 then begin  ; case i <= e (Eq.47)
    mue[w1] = (e2e[w1]-sin(psi[w1]/2)^2*e2i[w1])/(2-e1e[w1]-psi[w1]*e1i[w1]/!dpi)
  endif
  if w2[0] ne -1 then begin  ; case i >= e (Eq.51)
    mue[w2] = (cos(psi[w2])*e2i[w2]+sin(psi[w2]/2)^2*e2e[w2])/(2-e1i[w2]-psi[w2]*e1e[w2]/!dpi)
  endif

  xi = 1./sqrt(1.+!dpi*tantheta^2)
  mue = (mu+se*tantheta*mue)*xi

  if cos then return, mue else return, acos(mue)/!dpi*180
